...
class NetworkModuleTest {
    ...
    private val potholeJson =
        """{"id":"srv-1","latitude":-23.5,"longitude":-46.6,"severity":18.5,"status":"PENDING","readingCount":2,"distinctReporterCount":1,"createdAt":"2026-09-01T12:00:00Z"}"""
    private val cityJson =
        """{"ibgeCode":3550308,"name":"São Paulo","state":"SP","totalPotholes":10,"fixedPotholes":2,"recurrenceCount":1,"rank":4}"""
    ...
    @Test
    fun `submitReading posts the reading with the reporter token header`() = runTest {
        enqueueJson(potholeJson)

        val response = potholeService.submitReading("token-1", PotholeReadingRequestDto("c1", -23.5, -46.6, 18.5f, 1000L))
        ...
        assertEquals(PotholeResponseDto("srv-1", -23.5, -46.6, 18.5f, "PENDING", 2, 1, "2026-09-01T12:00:00Z"), response)
    }

    @Test
    fun `submitReadings posts a batch and parses per-item results`() = runTest {
        enqueueJson("""[{"clientId":"c1","pothole":$potholeJson},{"clientId":"c2","error":"timestamp is too old"}]""")
    ...
    @Test
    fun `getPotholes sends the viewport as query params`() = runTest {
        enqueueJson("[$potholeJson]")

        val result = potholeService.getPotholes(minLat = -24.0, minLon = -47.0, maxLat = -23.0, maxLon = -46.0, limit = 50)
    ...
    @Test
    fun `getNearbyPotholes omits optional params when not given`() = runTest {
        enqueueJson("[]")
    ...
    @Test
    fun `castFixVote and deletePothole address the pothole by id with the token`() = runTest {
        enqueueJson(potholeJson)
    ...
    @Test
    fun `city ranking and nearest city parse the backend shapes`() = runTest {
