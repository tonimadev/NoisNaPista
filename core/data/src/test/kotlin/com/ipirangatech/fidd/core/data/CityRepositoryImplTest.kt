...
class CityRepositoryImplTest {
    ...
    private class RecordingCityService(private val fail: Boolean = false, private val row: CityRankingResponseDto) : CityService {
        val sortParams = mutableListOf<String>()

        override suspend fun getRanking(sortBy: String): CityRankingListResponseDto {
            sortParams += sortBy
            if (fail) error("offline")
            return CityRankingListResponseDto(totalCities = 5, top = listOf(row), bottom = listOf(row.copy(rank = null)))
        }

        override suspend fun getNearestCity(lat: Double, lon: Double, sortBy: String): CityRankingResponseDto {
            sortParams += sortBy
            if (fail) error("offline")
            return row
        }
    ...
    @Test
    fun `each sort metric maps to the backend query value`() = runTest {
    ...
    @Test
    fun `ranking and nearest city are mapped to the domain model`() = runTest {
    ...
    @Test
    fun `service errors become failed results`() = runTest {
