...
import digital.tonima.noisnapista.core.sensor.MotionSensor
...
class PotholeDetectorTest {
    ...
    private fun fakeMotionSensor(
        ...
        override fun getGravityUpdates(): Flow<AccelerationSample> =
            gravitySample?.let { flow { emit(it); awaitCancellation() } } ?: emptyFlow()

        override fun getRotationRateUpdates(): Flow<RotationRateSample> =
            rotationSample?.let { flow { emit(it); awaitCancellation() } } ?: emptyFlow()
    }
    ...
    @Test
    fun `detects an impact via the gravity-corrected vertical axis when the phone is mounted sideways`() = runBlocking {
        ...
        val sidewaysImpact = AccelerationSample(x = 30f, y = 0f, z = 0f, timestampNanos = 0L) // gravity + ~20 m/s² bump, all on X
        ...
        val pothole = withTimeoutOrNull(1000.milliseconds) { detector.potholes.first() }
        detector.stopDetection()
        assertNotNull("a bump aligned with a non-vertical raw axis should still be detected once corrected for orientation", pothole)
    }

    @Test
    fun `does not detect an impact while the gyroscope shows the phone is being handled`() = runBlocking {
        val highRotation = RotationRateSample(x = 0f, y = 0f, z = 20f, timestampNanos = 0L) // > HANDLING_ROTATION_THRESHOLD_RAD_S (12f)
        val detector = PotholeDetector(
