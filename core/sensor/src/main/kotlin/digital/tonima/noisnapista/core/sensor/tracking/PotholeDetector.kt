...
class PotholeDetector @Inject constructor(
    ...
    private val _latestGravity = MutableStateFlow<AccelerationSample?>(null)
    ...
    private val _latestRotationRate = MutableStateFlow<RotationRateSample?>(null)
    ...
    fun startDetection() {
        ...
        detectionScope.launch {
            motionSensor.getGravityUpdates()
                .catch { }
                .collect { _latestGravity.value = it }
        }

        detectionScope.launch {
            motionSensor.getRotationRateUpdates()
                .catch { }
                .collect { _latestRotationRate.value = it }
        }
        ...
        detectionScope.launch {
            motionSensor.getAccelerationUpdates()
                ...
                .collect { sample ->
                    ...
                    val impactMagnitude = computeVerticalAcceleration(sample, _latestGravity.value)
                    ...
                    if (impactMagnitude > IMPACT_THRESHOLD && !withinCooldown) { // Limiar de impacto
                        _currentLocation.value?.let { location ->
                            ...
                            val isLikelyHandling = _latestRotationRate.value?.let { rotationMagnitude(it) > HANDLING_ROTATION_THRESHOLD_RAD_S } == true
    ...
    private fun computeVerticalAcceleration(sample: AccelerationSample, gravity: AccelerationSample?): Float {
    ...
    fun stopDetection() {
        ...
        _latestGravity.value = null
        _latestRotationRate.value = null
