...
class MapViewModelTest {
    ...
    @Test
    fun `only active local potholes are shown`() = runTest {
    ...
    @Test
    fun `before tracking the preview fix is the current location, then the live stream takes over`() = runTest {
    ...
    @Test
    fun `nothing is fetched until the map reports a viewport`() = runTest {
        val vm = viewModel()
    ...
    @Test
    fun `viewport changes load community potholes and failures keep the previous pins silently`() = runTest {
        val vm = viewModel()
    ...
    @Test
    fun `a manual refresh reports failures but not superseded requests`() = runTest {
        val vm = viewModel()
        vm.onViewportChanged(viewport)
    ...
    @Test
    fun `recenter uses the live fix while tracking`() = runTest {
        val vm = viewModel()
    ...
    @Test
    fun `recenter fetches a fresh fix when idle, or explains that there is none`() = runTest {
        val vm = viewModel()
